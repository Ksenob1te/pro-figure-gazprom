...
class AchievementController(Controller):
    ...
    async def get_user_achievements(self, user: GetUser):
        user_stats = self.achievement_service.get_create_user_stats(user.user_id)
        user_achievements =  self.achievement_service.get_user_achievements(user.user_id)
        unearned_achievements = self.achievement_service.get_unearned_achievements(user.user_id)
    ...
    async def assign_achievement(self, achievement_code: str, user: GetUser):
        ...
        await self.achievement_service.grant_achievement(user.user_id, achievement.code)
